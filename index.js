...
async function connectToWA() {
  console.log("Connecting GAIYA Goldern Bot 🧬...");
  const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, '/auth_info_baileys/'));
  const { version } = await fetchLatestBaileysVersion();

  const Gaiya = makeWASocket({
  ...
  danuwa.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      const up = `Gaiya Goldern Bot connected ✅\n\nPREFIX: ${prefix}`;
      await danuwa.sendMessage(ownerNumber[0] + "@s.whatsapp.net", {
        image: { url: `https://github.com/DANUWA-MD/DANUWA-MD/blob/main/images/DANUWA-MD.png?radanu` },
        caption: up
      });
...
app.get("/", (req, res) => {
  res.send("Hey, Gaiya Goldern Bot started✅");
});
